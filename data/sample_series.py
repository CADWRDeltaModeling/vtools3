...
from vtools.functions import butterworth
...
def bessel_df():
    ...
    numpoints=100000
    date_rng1 = pd.date_range(start='1992-03-07',periods=numpoints,freq='15min')
    date_rng2 = pd.date_range(start='1992-03-07',periods=numpoints,freq='15min')
...
def small_subtide():
    ...
    fortnight_nsec = 14*86400
    t = np.arange(0,fortnight_nsec,900)
    nsample = len(t)
    FLOW_SCALE = 100000.
    tide = t*0.
    ...
    subtide_freq = 1./(3.*86400.) #one cycle per 3 days
    # Add a subtide that is very small compared to the tidal amplitude
    tide += FLOW_SCALE*np.cos(subtide_freq*t*2.*np.pi)/100.

    dr = pd.date_range(start="2000-01-01",periods=nsample,freq="15min")
    return pd.DataFrame({"tide":tide},dr)
...
if __name__ == "__main__":
    ...
    ts = tough_subtide()
    filtered = butterworth(ts,pd.DateOffset(hours=40))
