...
def ts_blend(series, names=None, blend_length=None):
    ...
    blend_length : int or Timedelta-like, optional
        ...
        - If a Timedelta-like value (e.g. '2h', pd.Timedelta('30min')), then
          blending is performed in time units on the regular working index.

        If None or non-positive, ts_blend behaves like a hard-priority merge
        with no soft shoulder blending.

    Raises
    ------
    ValueError
        If the inputs do not have a common regular frequency.
    ...
    # Align all to working index and normalize to DataFrames
    aligned = []
    for s in series:
