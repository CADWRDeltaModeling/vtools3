    ...
    if max_absent_frac is not None:
        try:
            ...
            if period == months(1):
                ...
                invalid = ts1['count'] < max_absent_frac * \
                    ts.index.days_in_month
            else:
                raise ValueError(
                    "Offset {} not supported. Only absolute offsets (<= 1day) or one month averages supported with max_absent_frac option".format(period))
    else:
